...
// Cria uma Feature no Azure, vinculada a um parent (Epic ou Iniciativa)
async function createFeature(b) {
  const area = SQUAD_AREA[b.squadId];
  if (!area) { const e = new Error('squad inválida'); e.status = 400; throw e; }
  if (!b.title) { const e = new Error('title obrigatório'); e.status = 400; throw e; }
  const ops = [
  ...
  if (b.parentId) ops.push({ op: 'add', path: '/relations/-', value: {
    rel: 'System.LinkTypes.Hierarchy-Reverse', url: `https://dev.azure.com/${ORG}/_apis/wit/workItems/${b.parentId}` } });
  const created = await azdo(`wit/workitems/$Feature`, { method: 'POST', body: ops, contentType: 'application/json-patch+json' });
  return { id: created.id, title: created.fields['System.Title'], parent: b.parentId || null };
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (parts[0] === 'workitems') {
      ...
      // POST /workitems/feature
      if (parts[1] === 'feature' && method === 'POST') return ok(await createFeature(body));
